...
f1 = open('tf_list.txt', 'r')
regulators = f1.read().splitlines()
f1.close()

start= time.time()
#print("gene",x, "is a target", end='\n')
(VIM, alphas, prediction_score, stability_score, treeEstimators) = dynGENIE3(TS_data,time_points, gene_names, nthreads=4)
end= time.time()
...
start2= time.time()
get_link_list(VIM, gene_names=gene_names,file_name='rankingAll')
get_link_list(VIM, gene_names=gene_names,regulators=regulators, file_name='ranking_regulators')
end2 = time.time()
